...
**Path A — browser only, no terminal.** The schema ships as four files,
because the Supabase SQL Editor rejects one large paste with "Query is too
large to be run via the SQL Editor". For each of

1. `supabase/production_setup_part1.sql` — types and tables
2. `supabase/production_setup_part2.sql` — core functions and financial RPCs
3. `supabase/production_setup_part3.sql` — reporting views and closings
4. `supabase/production_setup_part4.sql` — security, alerts, reference data

open the file on GitHub, use the **Copy raw file** button, paste it into the
Supabase dashboard's **SQL Editor**, and **Run** — strictly in order, waiting
for each part to succeed before the next. Together the parts are every
migration concatenated in order, generated by
`scripts/build-production-setup.sh`; they contain no demo data and create no
accounts. If a part fails partway, do not retry against the half-built
database — delete the project and start again from part 1 on a fresh one.
A NOTICE about the storage bucket at the end of part 4 is expected — hosted
Supabase reserves storage rules for the dashboard, so create them there:

1. **Storage → New bucket** — name `documents`, keep it **private**, file size
   limit 25 MB.
2. **Storage → documents → Policies → New policy** four times, all for the
   `authenticated` role:

| Policy name | Operation | Expression (USING / WITH CHECK) |
|---|---|---|
| `documents_read` | SELECT | `bucket_id = 'documents' and auth_role() is not null` |
| `documents_write` | INSERT | `bucket_id = 'documents' and can_write()` |
| `documents_update` | UPDATE | `bucket_id = 'documents' and can_write()` |
| `documents_delete` | DELETE | `bucket_id = 'documents' and is_ceo()` |
...
Then create the storage bucket and policies in the dashboard as in Path A —
the migration skips them on hosted Supabase for the same ownership reason.

Neither path runs `seed.sql`, which is what you want.
...
Test it — from the browser (the route also accepts the secret as a query
parameter), or with curl:

```
https://your-app.vercel.app/api/cron?secret=YOUR-CRON-SECRET
```
