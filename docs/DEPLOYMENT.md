...
**Path A — browser only, no terminal.** Open the repository on GitHub, navigate
to `supabase/production_setup.sql`, and use the **Copy raw file** button. In the
Supabase dashboard open **SQL Editor**, paste the whole thing, and **Run** once.
The file is every migration concatenated in order, generated by
`scripts/build-production-setup.sh`; it contains no demo data and creates no
accounts. A NOTICE about the storage bucket at the end is expected — hosted
